...
class BluetoothDeviceRepositoryTest : BaseTest() {
    ...
    fun setup() {
        ...
        repository = BluetoothDeviceRepository(thermometerRepository, testScope)
    }

    @Test
    fun `loadSavedDevices should load bluetooth devices and update status list`() {
        runTest {
            // Given
    ...
    @Test
    fun `loadSavedDevices should filter non-bluetooth devices`() {
        runTest {
            // Given
            val zigbeeDevice = Device.Basic(
                id = java.util.UUID.randomUUID(),
            ...
            val devices = listOf(testDevice, zigbeeDevice)
            coEvery { thermometerRepository.getDevices() } returns Result.Success(devices)
            ...
            repository.deviceConnectionStatusList.test {
                ...
                statusMap.containsKey(zigbeeDevice.macAddress) shouldBe false
            }
        }
    }
    ...
    @Test
    fun `saveNewDevice should not save if device already saved`() {
        runTest {
            // Given
            val devices = listOf(testDevice)
            coEvery { thermometerRepository.getDevices() } returns Result.Success(devices)
            repository.loadSavedDevices()
            testScheduler.advanceUntilIdle() // Wait for coroutines to complete

            // When
            repository.saveNewDevice(testDevice)

            // Then
            coVerify(exactly = 0) { thermometerRepository.saveDevice(any()) }
            coVerify(exactly = 0) { thermometerRepository.saveThermometerData(any()) }
        }
    }
